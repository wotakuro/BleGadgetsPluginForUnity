...
DllExport int32_t _BlePluginScanGetDeviceServiceCount(int idx) {
	BleDeviceWatcher& watcher = BleDeviceWatcher::GetInstance();
	return watcher.GetServiceNum(idx);
}
DllExport UuidHandle _BlePluginScanGetDeviceServiceUuid(int idx, int serviceIdx) {
	BleDeviceWatcher& watcher = BleDeviceWatcher::GetInstance();

	WinRtGuid guid;
	if (watcher.GetServiceUuid(idx, serviceIdx, guid)) {
