...
int BleDeviceWatcher::GetServiceNum(int idx)const {
	if (idx < 0 || idx >= m_cacheData.size()) {
		return 0;
	}
...
bool BleDeviceWatcher::GetServiceUuid(int idx, int serviceIdx, WinRtGuid& dest)const {
	if (idx < 0 || idx >= m_cacheData.size()) {
		return false;
	}
	if (serviceIdx < 0 || serviceIdx >= m_cacheData.at(idx).serviceUuids.size()) {
		return false;
	}
	dest = m_cacheData.at(idx).serviceUuids.at(serviceIdx);
	return true;
}
