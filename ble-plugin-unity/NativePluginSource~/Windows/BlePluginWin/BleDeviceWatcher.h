...
//			std::string name;
			...
			int rssi;
			clock_t lastFound;
			std::vector< WinRtGuid > serviceUuids;
			...
			void ClearServiceUuid() {
				serviceUuids.clear();
			}
			void AppendServiceUuid(const WinRtGuid &servide) {
				serviceUuids.push_back(servide);
			}
	...
	public:
		...
		int GetServiceNum(int idx)const;
		bool GetServiceUuid(int idx, int serviceIdx, WinRtGuid& dest)const;
